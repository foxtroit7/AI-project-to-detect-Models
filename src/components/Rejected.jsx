...
import { FaEye, FaClock, FaCamera, FaUserShield, FaMapMarkerAlt, FaRegCalendarAlt, FaBook, FaStop } from "react-icons/fa";
import "../css/users.css";
import { useNavigate } from "react-router-dom";

const Rejected = () => {
  const [rejectedData] = useState([
    {
      id: 1,
      ...
      <Container>
        <div className="d-flex justify-content-center align-items-center mb-4">
          <h2 className="fw-bold text-center">
            <span style={{ fontWeight: "bold", color: "#dc2121ff" }}>Rejected Detections</span>
          </h2>
        </div>
          ...
          <Col md={3}>
            <InputGroup>
              ...
              >
                ...
                {[...new Set(rejectedData.map(item => item.modelName))].map((model, idx) => (
          ...
          <Col md={3}>
            <InputGroup>
              ...
              >
                ...
                {[...new Set(rejectedData.map(item => item.cameraName))].map((cam, idx) => (
          ...
          <Col md={3}>
            <InputGroup>
              ...
              >
                ...
                {[...new Set(rejectedData.map(item => item.zone))].map((z, idx) => (
          ...
          <Col md={3}>
            <InputGroup>
              ...
              >
                ...
                {[...new Set(rejectedData.map(item => item.caption))].map((cap, idx) => (
        ...
        <Table className="user-table" responsive>
          <thead>
            <tr>
              ...
              <th className="text-center"><FaUserShield className="me-2" />Rejected By</th>
              <th className="text-center"><FaRegCalendarAlt className="me-2" />Caption</th>
              <th className="text-center"><FaStop className="me-2" />Status</th>
              <th className="text-center"><FaEye className="me-2" />View</th>
            </tr>
          </thead>
          <tbody>
            {filteredData.map((item) => (
              <tr key={item.id}>
                ...
                <td className="text-center">{item.rejectedBy}</td>
                <td className="text-center">
                  <div className="camera-scroll d-flex overflow-auto">
                    <span className="badge bg-danger me-2">{item.caption}</span>
                  </div>
                </td>
                <td className="text-center" style={{ fontWeight: "bold", color: "#dc212dff" }}>{item.status}</td>
                <td>
                  <button
                    className="btn d-flex align-items-center justify-content-center ms-3"
                    style={{ fontWeight: "bold", backgroundColor: "#dc2131ff" }}
                    onClick={() => navigate(`/single-reject/${item.id}`)}
                  >
                    <FaEye />
                  </button>
                </td>
